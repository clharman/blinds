...
int clockwiseThreshold = 140; // change this value to set the point at which the blinds will change direction from clockwise to anticlockwise. Ranges from 0 - 1023
int antiClockwiseThreshold = 850;// change this value to set the point at which the blinds will change direction from anticlockwise to clockwise
int roomNoiseThreshold = 70; //change this value to set the threshold at which the microphone will begin listening. Ranges from 0 - 1023 - 75 seems to be optimal
...
void loop(){
  ...
  // swaps mode between 0 and -1 every 10000 ms (10 seconds)
  if(millis() % 10000 == 0){
    mode = ~mode;
  }
  ...
  if(mode == 0){
    ...
    if(rawData1 < roomNoiseThreshold){ //if mic does not exceed room noise, then don't read mic
      inputData = 0;
    }
    //   
    ...
    smoothedVal = smoothData1 * 0.25; // divide the smoothed values by 4 so that they fit within a suitable output range for the motor
